...
def genPersona():
    ...
    persona_prompt = ['Act as a highly experienced and extremely competent medical oncologist from the world-renowned Princess Margaret Cancer Centre in Toronto, Ontario. ',
...
def genDeid():
    ...
    deid_prompt = ("The following tags are used to replace the protected health information in the de-identified note.\n " +
...
def genTarget(target_string):
    ...
    target_prompt = ("Your task is to predict the probability that a patient undergoing systemic therapy for cancer will " + target_prompt + 
...
def genHealthFactors(target_string):
    ...
    if target_string == 'target_death_in_365d':
        health_string = ("Consider all relevant factors, including the patient's current treatment regimen, " + 
...
def genCOT():
    ...
    cot_prompt = ("Use chain of thought reasoning to logically deduce the patient's risk. " +
...
def genReasonExample(numeric_proba):
    ...
    reason_prompt = reason_prompt + ("""{'Reason': "Based on the patient's advanced age (70), extensive liver metastases, and rapid deterioration, """ +
...
def genPrompts(target_name, numeric_proba, save_dir):
    ...
    format_prompt = ("""I am going to give you a template for your output. Words in angle brackets are my placeholders. Fill in my placeholders with your output. """ +
                    """Please preserve the overall formatting of my template. My template is:\n {'Reason': <REASON>, 'Probability': <PROBABILITY>}\n""" +
                    """<REASON> must be a very concise explanation of how you arrived at the predicted probability enclosed in double quotes. """)
        ...
        for health_val in health_prompt:
            for add_deid in [0,1]:
                ...
                for cot_param in [0,1]:
                    if cot_param == 1:
                        prompt = persona_val + target_prompt + health_val + deid_prompt + cot_prompt + format_prompt + proba_prompt + format_end_prompt
                    else:
                        prompt = persona_val + target_prompt + health_val + deid_prompt + format_prompt + proba_prompt + format_end_prompt + reason_prompt
...
if __name__ == "__main__":
    ...
    parser.add_argument("numeric_proba", help="numeric value for probability", type=int)  # numeric value for probability?
