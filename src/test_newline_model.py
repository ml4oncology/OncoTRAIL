...
from config import bayesopt_param, model_static_param, model_tuning_param, start_test_date
...
def test_newline_model( notesPath, splitConfig, hyperParamEval, modelName, modelDir, resultsDir, LLMPath, targetName ):

    # adjust parameter values
    if 'early_stopping_rounds' in model_static_param['XGB']: del model_static_param['XGB']['early_stopping_rounds']
    if 'early_stopping_rounds' in model_static_param['LGBM']: del model_static_param['LGBM']['early_stopping_rounds']

    # save string for file
    target_name_nospace = targetName.replace("_","-")
    file_save_str = f'{modelName}_{splitConfig}_{hyperParamEval}_{target_name_nospace}'
    ...
    df = df.loc[df[targetName] != -1].copy()
    df['new_line_count'] = df['note'].apply(lambda x: x.count('\n'))
    tokenizer = AutoTokenizer.from_pretrained(LLMPath)
    df['tokenized_len'] = df['note'].apply(lambda x: len(tokenizer.tokenize(x)))
    df['newline_prop_tokenized_len'] = df['new_line_count']/df['tokenized_len']
    ...
    colsToKeep = ['new_line_count', 'tokenized_len', 'newline_prop_tokenized_len']
    scaler = StandardScaler()
    X_train = scaler.fit_transform(train_eval_data[colsToKeep].to_numpy())
    X_valid = scaler.transform(valid_data[colsToKeep].to_numpy())
    X_test = scaler.transform(test_data[colsToKeep].to_numpy())
    Y_train = train_eval_data[targetName].to_numpy()
    Y_valid = valid_data[targetName].to_numpy()
    Y_test = test_data[targetName].to_numpy()
...
if __name__ == "__main__":
    ...
    test_newline_model( args.notesPath, args.splitConfig, args.hyperParamEval, args.modelName, args.modelDir, args.resultsDir, args.LLMPath, args.targetName )