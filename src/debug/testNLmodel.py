...
def test_newline_model(
    ...
    if "early_stopping_rounds" in model_static_param["XGB"]:
        del model_static_param["XGB"]["early_stopping_rounds"]
    if "early_stopping_rounds" in model_static_param["LGBM"]:
        del model_static_param["LGBM"]["early_stopping_rounds"]
    ...
    algs = {"LR": LogisticRegression, "XGB": XGBClassifier, "LGBM": LGBMClassifier}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "splitConfig", help="configuration of train-valid-test split", type=str
    )  # configuration of train-valid-test split
    parser.add_argument(
        "hyperParamEval", help="function for hyperparameter evaluation", type=str
    )  # hyperparameter evaluation function
    parser.add_argument(
        "modelName", help="model name", type=str
    )  # name of machine learning model
    parser.add_argument(
        "modelDir", help="model directory", type=str
    )  # directory to save model
    parser.add_argument(
        "resultsDir", help="results directory", type=str
