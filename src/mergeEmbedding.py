...
def mergeEmbedding(data_dir, data_file_name, save_dir, num_files):
    # initialize the dictionary here
    target_embedding_list = {}

    for idx in range(num_files):
        with np.load(f"{data_dir}/{data_file_name}_part{idx}.npz") as data:
    ...
    np.savez(f"{save_dir}/{data_file_name}.npz", **target_embedding_concat)
...
if __name__ == "__main__":
    ...
    parser.add_argument("num_files", help="number of files", type=int)  # number of files
    args = parser.parse_args()

    mergeEmbedding(args.data_dir, args.data_file_name, args.save_dir, args.num_files)
