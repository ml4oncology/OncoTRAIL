...
def processDataFramePromptingVariation(data_path, save_dir, num_mrns, num_replicates):
    ...
    clinical_notes_filter = clinical_notes.sample(n=clinical_notes.shape[0],random_state=100).groupby(['mrn']).first().reset_index(drop=True).sample(n=num_mrns,random_state=100).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "num_mrns", help="number of mrns", type=int
    )  # number of mrns
    parser.add_argument(
    ...
    processDataFramePromptingVariation(args.data_path, args.save_dir, args.num_mrns, args.num_replicates)
