...
# need to figure out how tutorial would change if gguf model is used
...
def main(
    ...
    # only keep the first 100 rows of train_set_df
    train_set_df = train_set_df.head(100)
    ...
    # ====================================================================================
    # Load base model & tokenizer with FastLanguageModel
    # ====================================================================================
    ...
    # ====================================================================================
    # Trim the classification head to only the tokens “0” and “1”
    # (We detect the token‐IDs for strings “0” and “1” via tokenizer.encode)
    # ====================================================================================
    number_token_ids = []
    ...
    # Keep only those two rows out of lm_head.weight
    par = torch.nn.Parameter(model.lm_head.weight[number_token_ids, :])

    old_shape = model.lm_head.weight.shape  # (vocab_size, hidden_size)
    old_size = old_shape[0]                 # original vocab size

    model.lm_head.weight = par

    # Build a reverse map: old_token_id → new lm_head index (0 or 1)
    reverse_map = {value: idx for idx, value in enumerate(number_token_ids)}

    # ====================================================================================
    # Wrap model in PEFT / LoRA (unchanged except “target_modules” may differ)
    # ====================================================================================
    ...
    collator = DataCollatorForLastTokenLM(
        ...
        mlm=False,
        ignore_index=-100,
        reverse_map=reverse_map,  # ⇨ pass it here
    )
    ...
    training_args = TrainingArguments(
        ...
        seed=3407,
        output_dir=os.path.join(results_dir, target_name),
        num_train_epochs=1,
        report_to="none",
    ...
    # # ====================================================================================
    # # Re‐construct the old lm_head so you can still generate “0” or “1”
    # # ====================================================================================
    ...
    # new_lm_head = torch.full((old_size, hidden_dim), 0.0, dtype=trimmed_lm_head.dtype, device=trimmed_lm_head.device)
    ...
    # If you want to save to CSV in your output directory:
    os.makedirs(results_dir, exist_ok=True)
    csv_path = os.path.join(results_dir, f"{target_name}_test_predictions.csv")
