...
def processPromptEngineering(data_dir, LLM_path, LLM_name, save_dir, target_name, file_name,
                             patient_num, json_file, prompt_num, num_samples):

    torch.manual_seed(0)
    target_name_nospace = target_name.replace("_","-")
    file_name_notemplate = file_name.replace("_template","")
    print(f"{file_name_notemplate}_{target_name_nospace}_{LLM_name}_patient{patient_num}_prompt{prompt_num}")
    ...
    results_df.to_csv(f'{save_dir}/{file_name_notemplate}_{target_name_nospace}_{LLM_name}_patient{patient_num}_prompt{prompt_num}.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument("patient_num", help="patient number", type=int)  # patient number
    ...
    processPromptEngineering(args.data_dir, args.LLM_path, args.LLM_name, args.save_dir, args.target_name,
                      args.file_name, args.patient_num, args.json_file, args.prompt_num,
                      args.num_samples)
