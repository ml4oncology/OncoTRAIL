...
opis_data_path="/cluster/projects/gliugroup/2BLAST/data/final/data/raw/opis.parquet.gzip"
...
for config_name in "firstTreatmentOnly-medOnc-ConsultLetterClinic_deid" "firstVisitOnly-medOnc-ConsultLetterClinic_deid"; do # "mostRecentVisit-medOnc-ConsultLetterClinic_deid"; do
    pySLURMargs.py $userName $memory $condaEnv $nGPU $runTime "../src/prep/anchor_note_to_treatment.py $data_path $treatment_data_path $ed_visit_data_path $symptom_data_path $last_seen_data_path $lab_values_data_path $opis_data_path $save_dir $config_name $test_end_date $lookback_window $add_tabular_to_note" 
done
